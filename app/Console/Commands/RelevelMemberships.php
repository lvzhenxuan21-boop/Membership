...
class RelevelMemberships extends Command
{
    ...
    public function handle(): int
    {
        ...
        // 初始化从未设置过周期的档案（基线 = 当前累计成长值）；分块处理，避免全表载入内存
        $initialized = 0;
        MemberProfile::whereNull('period_started_at')->orderBy('id')
            ->chunkById(500, function ($profiles) use (&$initialized) {
        ...
        if ($initialized) {
            $this->info("初始化周期基线 {$initialized} 位会员");
        }

        // 到期重算：周期内成长值（growth - growth_base）决定本周期可享的最高等级
        $due = 0;
        $downgraded = 0;
        MemberProfile::with('level')
            ->whereNotNull('period_started_at')
            ->where('period_started_at', '<', $boundary)
            ->orderBy('id')
            ->chunkById(500, function ($profiles) use ($months, &$due, &$downgraded) {
                foreach ($profiles as $profile) {
                    $due++;
        ...
        $this->info("周期重算完成：处理 {$due} 位会员，降级 {$downgraded} 位");
        return self::SUCCESS;
    }
}
